...
export default async function Page(props: PageProps<"/challenge/[id]/retry">) {
  const { id } = await props.params;
  const challenge = CHALLENGE_BY_ID.get(id);
  if (!challenge) notFound();

  return challenge.kind === "metrics" ? (
    <MetricsAttempt challenge={challenge} attempt={2} />
  ) : (
    <AttemptForm challenge={challenge} attempt={2} />
  );
}
