...
/* ---------------------------------------------------------------------------
   ...
   Three buckets, three colours, and no ranking within them. Skip is styled as
   an achievement rather than an omission, because "you already have this" is
   the finding that saves the learner the most time.
--------------------------------------------------------------------------- */
...
export function GapMap() {
  ...
  return (
    <div className="screen screen-flush">
      ...
      <div role="group" aria-label="Filter capabilities" className="mt-4 flex gap-1.5">
        <button
          type="button"
          aria-pressed={filter === null}
          onClick={() => setFilter(null)}
          className={cn(
            "flex min-h-11 items-center justify-center rounded-[var(--radius-pill)] border px-3 text-[13px] font-semibold transition-colors",
            filter === null
        ...
        {ORDER.map((key) => {
          ...
          return (
            <button
              ...
              className={cn(
                "flex min-h-11 flex-1 items-center justify-center gap-1.5 rounded-[var(--radius-pill)] border px-2 text-[13px] font-semibold transition-colors",
